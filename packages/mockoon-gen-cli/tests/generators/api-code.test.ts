...
import { syncApiCodeToArtifact } from "../../src/generators/api-code-sync.js";
...
describe("generateApiCode", () => {
  it("does not add generated-file or eslint-disabling headers", () => {
    const code = generateApiCode(createArtifact());
    expect(code).not.toContain("eslint-disable");
    expect(code).not.toContain("generated by mockoon-gen");
  });
...
describe("syncApiCodeToArtifact", () => {
  it("uses edited VO fields and mapper assignments as the artifact source", () => {
    const artifact = createArtifact();
    const code = generateApiCode(artifact)
      .replace("userName: string;", "displayName: string;")
      .replace("vo.userName = dto.user_name;", "vo.displayName = dto.email;");

    const synced = syncApiCodeToArtifact(artifact, code);
    expect(synced.outputs.apiCode.origin).toBe("manual");
    expect(synced.outputs.apiCode.lastGeneratedSha256).toBeNull();
    expect(synced.endpoints[0]?.vo.fields).toEqual([
      expect.objectContaining({
        name: "displayName",
        type: "string",
        origin: "manual",
        sources: [{ path: "response.body.email", role: "displayName" }]
      })
    ]);
    expect(synced.endpoints[0]?.mapper.steps).toEqual([
      expect.objectContaining({ operation: "assign", inputs: ["response.body.email"], output: "vo.displayName" })
    ]);
  });
});
