...
function createFieldIdentifiers(fieldNames) {
...
function camel(value) {
  return value.replace(/[_-\s]+(.)?/g, (_, char) => char ? char.toUpperCase() : "").replace(/^[A-Z]/, (char) => char.toLowerCase());
}
...
var originSchema = z.enum(["generated", "inferred", "imported", "manual"]);
var reviewStatusSchema = z.enum(["unreviewed", "needs-change", "confirmed"]);
...
var reviewableSchema = z.object({
  origin: originSchema,
  reviewStatus: reviewStatusSchema
});
var sourceSchema = reviewableSchema.extend({
  id: z.string().min(1),
...
function generateEndpoint(endpoint, transformResponse) {
  ...
  return [
    ...
    transformResponse ? `  const dto = await request<${dto}>(${requestPath ? "path" : `"${endpoint.path}"`}, { method: "${endpoint.method}" });` : `  return request<${dto}>(${requestPath ? "path" : `"${endpoint.path}"`}, { method: "${endpoint.method}" });`,
...
function buildDtoShape(endpoint) {
...
// src/generators/api-code-sync.ts
function syncApiCodeToArtifact(artifact, code) {
...
function syncEndpoint(endpoint, code) {
  ...
  const fields = voFields.map((field) => {
    const previous = oldFields.get(field.name);
    const assignment = assignments.find((item2) => item2.field === field.name);
    const sourcePath = assignment?.source;
    const source = sourcePath ? { path: sourcePath, role: field.name } : void 0;
    return {
      ...previous ?? {
        name: field.name,
        type: field.type,
...
function extractInterface(code, name) {
...
function extractBalancedBlock(code, openIndex) {
  ...
  for (const line of block.split("\n")) {
    ...
    if (leaf) {
      ...
      result.push({
        ...
        type: leaf[2].trim(),
        path: [...stack.map((item2) => item2.name), parsePropertyName(leaf[1])]
      });
    }
  }
  return result;
...
function typescriptAccessorToResponsePath(expression) {
  if (expression === "undefined") return void 0;
  if (!expression.startsWith("dto")) return void 0;
...
function generateMockoonEnvironment(artifact) {
  ...
  return {
    ...
    routes: artifact.endpoints.map((endpoint) => ({
      ...
      enabled: true,
      responses: endpoint.mock.scenarios.filter((scenario) => scenario.enabled).map((scenario) => ({
...
// src/index.ts
var MOCKGEN_VERSION = "0.1.0";

// src/openapi/load-openapi.ts
var import_yaml = __toESM(require_dist(), 1);
import { readFile as readFile2 } from "node:fs/promises";
async function loadOpenApi(file) {
  const raw = await readFile2(file, "utf8");
  const document = import_yaml.default.parse(raw);
...
function createProgram() {
  ...
  program2.command("sync-api-code").description("Sync manually edited TypeScript API code back into api-artifact.json.").requiredOption("--from <artifact>").option("--cwd <cwd>", "Working directory", process.cwd()).action(async (options) => {
    const artifactFile = resolveFromCwd(options.cwd, options.from);
    const artifact = await readArtifact(artifactFile);
    const targetFile = artifact.outputs.apiCode.suggestedFile;
    const code = await readFile3(join(options.cwd, targetFile), "utf8");
...
async function readArtifact(file) {
  return artifactSchema.parse(JSON.parse(await readFile3(file, "utf8")));
}
