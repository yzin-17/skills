...
**用途**：以小任务、新上下文和最终统一 review 控制长任务的上下文累积。父代理负责需求、范围、关键决策、调度与最终验收；worker 负责有界执行和局部验证。Luna 调用成本按可忽略处理，不再为了省调用而维持长会话。

**适用门槛**：仅顶层非 Luna 编排者使用；任何模型的被委派 worker 都直接执行任务，不再触发本 skill 或创建子代理。父模型不绑定为 Sol，沿用用户选择。

**适用场景**：大量代码阅读、跨文件实现、测试、调试、日志或浏览器验证。简单本地低风险修改、小分析或小文档改动仍可直接完成。先对整体任务判断一次委派收益，拆小后不再逐项重复判断；用户点名模型只约束委派时的选择。

**教程**（完整规则见 [SKILL.md](skills/codex-cost/SKILL.md)；升级与配置问题按需查看 [Codex 运行时说明](skills/codex-cost/references/codex-runtime.md)）：

1. 每个任务只保留一个可验证交付物和一个主要执行面，附明确依赖；小范围实现、自检与测试可以一起完成，不把数据库、API、客户端和整体验收塞进一个大任务。
2. 沿用用户指定模型、角色和推理档位；非视觉工作默认 Luna，前端视觉、布局、样式及视觉验收禁止交给 Luna。核对角色实际加载的模型，而非只看角色名称。
3. 复用现有 spec/task 文档记录目标、约束、基线、任务状态、产物和验证证据；新 worker 只读自身任务与必要依赖，不复制完整历史对话。
4. 每个任务新建 worker 上下文，同一时刻最多一个打开的 worker；完成或安全交接后关闭，再创建下一个。可以复用角色配置，不复用会话；无法确认禁止历史继承时报告限制，不冒充已隔离。
5. worker 完成局部自检后返回简短状态。父代理中途只接收状态、更新清单和调度，不逐份审查 diff 或重跑测试；失败检查、契约冲突和越界需求立即阻止受影响的下游任务。
6. 全部实施任务完成后，新 worker 对组合后的最终状态执行必要的集成/回归验证；父代理随后统一 review 真实改动、跨任务契约和最新验证证据。`worker_done` 不等于最终验收通过。
7. 最终 review 发现的问题拆成小修复任务，用全新 worker 处理，修复批次完成后统一复核。检查未通过、验证缺失或仍有阻塞时不得声称整体完成。
